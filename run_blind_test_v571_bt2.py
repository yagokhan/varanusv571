#!/usr/bin/env python3
"""
run_blind_test_v571_bt2.py — Varanus v5.7.1 Blind Test 2

Tests best_params_v57.json on data the optimizer NEVER saw:
  Jan 24 2026 – Mar 09 2026

Approach:
  - Train one final model on ALL optimization data (Jan 2023 – Oct 2025)
  - Generate signals on blind test window
  - Run backtest with Dynamic Trailing Stop — no parameter changes, no retraining on blind data

Output:
  varanus/config/varanus_v571_blind2_trades.csv
  varanus/config/varanus_v571_blind2_summary.csv
"""
...
OUT_TRADES  = _HERE / "varanus" / "config" / "varanus_v571_blind2_trades.csv"
OUT_SUMMARY = _HERE / "varanus" / "config" / "varanus_v571_blind2_summary.csv"

TRAIN_CUTOFF = pd.Timestamp("2025-11-01", tz="UTC")   # hard cutoff (end of train)
BLIND_START  = pd.Timestamp("2026-01-24", tz="UTC")
...
BLIND_MONTHS = [(2026, 1), (2026, 2), (2026, 3)]
...
def _download_month(asset, year, month):
    ...
    try:
        ...
        print(f"    x {asset} {year}-{month:02d}: {e}")
        return pd.DataFrame()
...
def fetch_blind_data():
    """Download Jan 2026 – Mar 2026 1h data and merge into cache."""
    print("[+] Fetching blind test data (Jan 2026 – Mar 2026) ...")
...
def run():
    print("=" * 65)
    print("  Varanus v5.7.1 — Blind Test 2")
    print(f"  Train : Jan 2023 – Oct 2025 (full optimization region)")
    print(f"  Test  : Jan 24 2026 – Mar 09 2026 (never seen by optimizer)")
    ...
    # Split: train = up to Oct 31 2025, blind = Jan 2026 onward
    ...
    summary = {
        "period":             "Jan 24 2026 – Mar 09 2026",
    ...
    print("\n" + "=" * 65)
    print("  BLIND TEST RESULTS — v5.7.1 (Blind Test 2)")
    print(f"  Period : Jan 24 2026 – Mar 09 2026 (~44 days)")
    print("=" * 65)
